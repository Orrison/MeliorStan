...
- **Type**: `string`
- **Default**: `''`
- **Description**: A regex pattern matched against the method **name** (case-insensitive). Matching methods are skipped, so they are neither reported individually nor included in the class total. Example: `^(setUp|tearDown|boot)` skips common lifecycle methods.
...
- **Fundamental increments** (cost: `1` flat):
  ...
  - Direct recursion: a method calling itself (`$this->foo()`, `self::foo()`, `static::foo()`, or `foo()` for plain functions) adds **+1** (counted once per method, regardless of how many self-calls are made).

...
The average in this rule (`show_classes_complexity`) is the weakest of the three signals because trivial methods dilute it. If you only want one class-level cyclomatic check, prefer `ExcessiveClassComplexity`. The average still has a niche, flagging classes where the *typical* method is dense rather than the *total*, which is why both exist.

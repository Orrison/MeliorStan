...
- **Modern PHP syntax.** MeliorStan rules are written against current PHP language features. Many of them analyze syntax that PHPMD's equivalents do not, such as `match` expressions, enums, readonly properties, first-class callable syntax, and other PHP 8.x constructs.

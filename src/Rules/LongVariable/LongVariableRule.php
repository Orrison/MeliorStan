...
class LongVariableRule implements Rule
{
    ...
    public function __construct(
        ...
        $this->maximum = $this->config->getMaximum();
    ...
    /**
     * Process assignment expressions to check for long variable names
     *
     * @return RuleError[]
     */
    ...
    /**
     * Calculate the effective length of a variable name after subtracting prefixes and suffixes
     */
    protected function calculateEffectiveLength(string $name): int
    {
        $prefixes = $this->config->getSubtractPrefixes();
        $suffixes = $this->config->getSubtractSuffixes();

        $effectiveName = $name;

        // Subtract prefixes
        foreach ($prefixes as $prefix) {
            if ($prefix !== '' && strpos($effectiveName, $prefix) === 0) {
                $effectiveName = substr($effectiveName, strlen($prefix));

                // Only subtract one prefix (the first match)
                break;
            }
        }
        ...
        foreach ($suffixes as $suffix) {
            if ($suffix !== '' && substr($effectiveName, -strlen($suffix)) === $suffix) {
                $effectiveName = substr($effectiveName, 0, -strlen($suffix));

                // Only subtract one suffix (the first match)
                break;
            }
        }

        return strlen($effectiveName);
    }
}
